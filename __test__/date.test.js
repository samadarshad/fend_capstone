...
describe('compare dates', () => {
    ...
    it('compare dates of different format', done => {
        ...
        let res = compareAsc(parse(dateA_from_user, 'dd/mm/yyyy', new Date()), parse(dateB_from_weatherbitApi, weatherDateScheme, new Date()))
        expect(res).toBe(1)

        res = compareAsc(parse(dateB_from_weatherbitApi, weatherDateScheme, new Date()), parse(dateA_from_user, 'dd/mm/yyyy', new Date()))
        expect(res).toBe(-1)

        const dateC_from_weatherbitApi = "2021-01-31"
        res = compareAsc(parse(dateC_from_weatherbitApi, weatherDateScheme, new Date()), parse(dateA_from_user, 'dd/mm/yyyy', new Date()))
        expect(res).toBe(0)
        done()
    })    
